...
module.exports = {
    ...
    get_cos:function(list1, list2){ 
        var numerator = math.dot(list1, list2);
        var denominator = Math.sqrt(math.dot(list1,list1)) * Math.sqrt(math.dot(list2,list2));
            ...
            if (err) {
                ...
                if (result.length != 0){
                    ...
                    db.query(query_sub, function(err, result_sub){
                        ...
                        if(result_sub.length < 2){ // should be  >= 1, since the user is always in the DB
...
/*get_cos = function(list1, list2){ //this is completely incorrect
        var numerator = math.dot(list1, list2);
        var denominator = Math.sqrt(math.dot(list1,list1)) * Math.sqrt(math.dot(list2,list2));
        return (numerator / denominator);
    }
